...
export default function AttendanceoneSchool() {
  const { getSchoolAttendance } = useDataContext();
  ...
  useEffect(() => {
    const fetchAttendance = async () => {
      setIsLoading(true);
      try {
        const response = await getSchoolAttendance(schoolCode);
        console.log("Received data AttendanceoneSchool:", response);
  ...
  }, [getSchoolAttendance, schoolCode]);
